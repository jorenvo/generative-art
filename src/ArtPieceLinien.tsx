...
export class Linien extends ArtPiece {
    draw() {
        ...
        const rect_per_col = Math.floor(rect_per_row * (this.canvas.height / this.canvas.width));
        ...
        for (let row = 0; row < rect_per_col; row++) {
            ...
            for (let col = 0; col < rect_per_row; col++) {
                ...
                let random_scale =
                    (random_scale_row * random_scale_col) / (10 / this.canvas.state.parameterA);
                ...
                if (row < rect_per_col) {
                    const new_row_x =
                        col * rect_width +
                        (this.canvas.state.random_pool[random_index++] - 0.5) * random_scale;
                    const new_row_y =
                        row * rect_height +
                        rect_height +
                        (this.canvas.state.random_pool[random_index++] - 0.5) * random_scale;
